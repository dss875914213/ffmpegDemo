...
public:
	...
	~FFDemux();
	BOOL					Init(const string& filename); // 初始化
...
private:
	shared_ptr<AVFormatContext>	m_pFormatContext;		// 流媒体解析上下文
	...
	shared_ptr<SDL_cond>		m_continueReadThread;	// 读线程信号量
	shared_ptr<SDL_Thread>		m_readThread;			// 读线程
};
