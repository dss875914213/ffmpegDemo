...
static int open_video_playing(void* arg)
{
    ...
    is->videoPlay_tid = SDL_CreateThread(video_playing_thread, "video playing thread", is);
...
static int open_video_stream(player_stat_t* is)
{
    ...
    // 2. 创建视频解码线程
    is->videoDecode_tid = SDL_CreateThread(video_decode_thread, "video decode thread", is);
