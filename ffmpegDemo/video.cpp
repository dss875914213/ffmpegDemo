...
static int QueuePicture(PlayerStation* is, AVFrame* sourceFrame, double pts, double duration, int64_t pos)
{
	...
	videoFrame->sar = sourceFrame->sample_aspect_ratio;
	videoFrame->uploaded = 0;
	videoFrame->width = sourceFrame->width;
	videoFrame->height = sourceFrame->height;
...
static int VideoDecodeThread(void* arg)
{
	...
	while (1)
	{
		...
		if (gotPicture < 0)
			goto EXIT;
		duration = (frameRate.num && frameRate.den ? av_q2d(AVRational{ frameRate.den, frameRate.num }) : 0);// 当前帧播放时长
...
static double ComputeTargetDelay(double delay, PlayerStation* is)
{
	...
	if (!isnan(diff))
	{
		if (diff <= -syncThreshold) // 若视频时钟落后于同步时钟，且超过同步域值
			delay = FFMAX(0, delay + diff); // 当前帧播放时刻落后于同步时钟 (delay+diff<0)则delay = 0(视频追赶，立即播放),否则delay=deley+diff
		else if (diff >= syncThreshold && delay > AV_SYNC_FRAMEUP_THRESHOLD) // 视频时钟超前于同步时钟，且超过同步阈值，但上一帧播放时长超长
			delay += diff; // 仅仅校正为 delay += diff, 主要是 AV_SYNC_FRAMEDUP_THRESHOLD 参数的作用
		else if (diff >= syncThreshold) // 视频时钟超前于同步时钟，且超过同步域值
...
RETRY:
	...
	vp = FrameQueuePeekLast(&is->videoFrameQueue); // 当前帧：当前帧显示的帧
	...
	if (is->paused)
		goto DISPLAY;
	lastDuration = VpDuration(is, lastvp, vp); // 上一帧播放时长：vp->pts-lastvp->pts
	...
	if (FrameQueueNumberRemaining(&is->videoFrameQueue) > 1) // 队列中未显示帧数>1(只有一帧则不考虑丢帧)
	{
		...
		duration = VpDuration(is, vp, nextvp); // 当前帧 vp 播放时长 = nextvp->pts - vp->pts
		// 当前帧 vp 未能及时播放, 即下一帧播放时刻(is->frameTimer+duration)小于当前系统时刻(time)
