...
int open_audio_stream(player_stat_t* is)
{
	...
	// 2. 创建视频解码线程
	is->audioDecode_tid = SDL_CreateThread(audio_decode_thread, "audio decode thread", is);
...
static int open_audio_playing(void* arg)
{
	...
	if (!(audioDevice = SDL_OpenAudioDevice(NULL, 0, &wanted_spec, &actual_spec, SDL_AUDIO_ALLOW_FREQUENCY_CHANGE | SDL_AUDIO_ALLOW_CHANNELS_CHANGE)))
	{
